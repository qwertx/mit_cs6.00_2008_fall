...
def performSim(time, numTrials, drunkType):
    ...
    # below is for generating a scattered figure
        xys.append(f.getLoc().getCoords())
    for i in range(len(xys)):
        xs.append(xys[i][0])
        ys.append(xys[i][1])
    pylab.figure()
    pylab.scatter(xs, ys)
    # scatter generation end here
    return distLists
    
def ansQuest(maxTime, numTrials, drunkType):
    means = []
